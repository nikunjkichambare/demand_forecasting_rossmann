...
Streamlit App: A frontend interface (planned) to allow users to upload minimal input (Id + Sales) and get forecasts.
...
Place raw input CSVs in data/raw/ (example: sample_submission.csv).
...
Saves trained model to models/best_sales_model.pkl.
Saves feature list to models/train_features_list.pkl.

4.2) Forecasting - python src/forecast.py

Reads data/raw/sample_submission.csv (Id + Sales).
Generates daily forecasts for the next 6 weeks.
Dates are formatted as DD-MM-YYYY.
Negative predictions are capped at 0.
Saves output CSV to data/processed/forecast_6weeks_daily.csv.

4.3) Streamlit App - streamlit run app/app.py

Allows users to upload CSV with minimal input.
Generates forecasts using forecast.py.

Scripts Description

src/train_model.py : Trains the XGBoost model on processed data	Outputs best_sales_model.pkl and train_features_list.pkl
src/forecast.py : Generates 6-week daily sales forecasts	Reads raw CSV (Id + Sales), formats date as DD-MM-YYYY, caps negative predictions
...
Exploratory Data Analysis (notebooks/)
      │
      ▼
Feature Engineering (src/train_model.py)
      │
      ▼
Model Training (XGBoost) → Save Model (models/best_sales_model.pkl)
      │
      ▼
Forecasting (src/forecast.py)
      │
      ▼
6-Week Daily Sales Forecast CSV (data/processed/forecast_6weeks_daily.csv)
      │
      ▼
Streamlit App (app/app.py)
  └─ User uploads minimal CSV (Id + Sales)
